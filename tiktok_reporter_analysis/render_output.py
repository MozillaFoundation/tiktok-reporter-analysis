...
def generate_html_report(results_dir):
    ...
    merged_df = events_df.merge(descriptions_df[["video", "description"]], on="video", how="left")
    ...
    color_flag = True
    for video, group in merged_df.groupby("video"):
        rowspan = len(group)
        ...
        html_str += f"<tr>{timestamp_td}{event_name_td}"
